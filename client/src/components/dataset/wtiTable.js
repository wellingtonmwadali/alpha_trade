...
const CrudeOilTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://alpha-trade.onrender.com/api/crude-oil-wti');
  ...
  const filterData = () => {
    ...
    const filteredData = crudeOilData.filter(entry => new Date(entry.Date) >= past12Months);

    return showFullTable ? crudeOilData : filteredData;
  };
